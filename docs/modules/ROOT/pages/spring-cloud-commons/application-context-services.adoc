...
different `DataSource` implementation if you need it to be refreshed.  If you set `spring.cloud.refresh.never-refreshable`
in your application configuration and use `HikariDataSource` then you should add `com.zaxxer.hikari.HikariDataSource` to
the list of beans to not refresh.
...
* `POST` to `/actuator/env` to update the `Environment` and rebind `@ConfigurationProperties` and log levels.
  To enabled this endpoint you must set `management.endpoint.env.post.enabled=true` and set a regular expression to limit what properties may be set at `management.endpoint.env.post.valid-keys-regex`.
* `/actuator/refresh` to re-load the bootstrap context and refresh the `@RefreshScope` beans.
...
it's critical to ensure that the endpoint is secured, monitored and valid keys to update are set to prevent potential security risks.
