...
Examples of `DiscoveryClient` implementations include https://docs.spring.io/spring-cloud-netflix/reference/{spring-cloud-version}[Spring Cloud Netflix Eureka], https://docs.spring.io/spring-cloud-consul/reference/{spring-cloud-version}[Spring Cloud Consul Discovery], and https://docs.spring.io/spring-cloud-zookeeper/reference/{spring-cloud-version}[Spring Cloud Zookeeper Discovery].
...
By default, the indicator invokes the client's `getServices` method. In deployments with many registered services it may be too
...
For the non-reactive version (with `RestTemplate`), you can enable it by adding link:https://github.com/spring-projects/spring-retry[Spring Retry] to your application's classpath. For the reactive version (with `WebClient`), you need to set `spring.cloud.loadbalancer.retry.enabled=true`.
...
create a bean of type `LoadBalancedRetryFactory` and return the `RetryListener` array
...
public class MyConfiguration {
    @Bean
    LoadBalancedRetryFactory retryListenerFactory() {
        return new LoadBalancedRetryFactory() {
            @Override
...
If `spring-webflux` is on the classpath, `LoadBalancedExchangeFilterFunction`
...
HasFeatures localFeatures() {
  return new HasFeatures(
    List.of(Something.class, Somethingelse.class),
    List.of(new NamedFeature("Some Other Feature", Someother.class)));
}
----
