...
Spring Cloud LoadBalancer allows you to set `String` hints that will be passed to the LoadBalancer within the `Request` object and that can later be used in `ReactiveLoadBalancer` implementations that can handle them.
...
One type of beans that it may be useful to register using <<custom-loadbalancer-configuration,Custom LoadBalancer configuration>> is `LoadBalancerLifecycle`.

The `LoadBalancerLifecycle` beans provide callback `onStart(Request<RC> request)` and `onComplete(CompletionContext<RES, T> completionContext)` methods that you should implement to specify what actions should take place before and after load-balancing.

`onStart(Request<RC> request)` takes as a `Request` object as a parameter. It contains data that will be used to select an appropriate instance, including the downstream client request and <<spring-cloud-loadbalancer-hints,hint>>. To the `onComplete(CompletionContext<RES, T> completionContext)` method, on the other hand, a `CompletionContext` object is provided that contains the LoadBalancer `Response`, including the selected service instance, the `Status` of the request executed against that service instance and, if available, the response returned to the downstream client and, if an exception has occurred, the corresponding `Throwable`.
