...
It is possible to set up the LoadBalancer in such a way that it will prefer the instance that was previously selected, if that instance is available.

For that, you will need to use the `SameInstancePreferenceServiceInstanceListSupplier`. It can be configured either by setting the value of `spring.cloud.loadbalancer.configurations` to `same-instance-preference` or by providing your own `ServiceInstanceListSupplier` bean, for example:
