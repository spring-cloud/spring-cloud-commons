...
* https://github.com/ben-manes/caffeine[Caffeine-backed]
* https://github.com/stoyanr[Evictor-backed]
...
The default `initialCapacity` is `256`.

If you do not have Caffeine in the classpath, but have `com.stoyanr:evictor` in the classpath, Evictor-based implementation will be used.
The default setup includes `evictMs` set to 30 seconds and the default `initialCapacity` is `256`.
...
WARN: Passing your own Caffeine specification will override any other LoadBalancerCache settings, including `ttl` and `capacity`.
