...
WARNING: `WeightedLoadBalancer` will modify the current weight of `ServiceInstance` to ensure that the next `choose` can
pick a lower weight instance, so you should ensure that the same instance for the same instanceId every time `choose` is called.
