...
mechanism, `ReactiveLoadBalancer` interface has been added and a *Round-Robin-based*, *Wieghted* and *Random* implementations
...
To calculate the weight of the `ServiceInstance`, we provide `WeightedServiceInstance` interface which extends `ServiceInstance`.
`WeightedServiceInstance` is stateful, we use `getWeight` to get the raw weight of the instance, use `getCurrentWeight` to get and modify the current weight of the instance.
The current weight of the instance will be changed when `choose` the best instance.
...
WARNING: Because `WeightedServiceInstance` is stateful, must use the same instance for the same instanceId every time `choose`,
unless the cache used by `DiscoveryClient` expired.

WARNING: The `ServiceInstanceListSupplier` used to get available instances must supplier the list of `WeightedServiceInstance`.
