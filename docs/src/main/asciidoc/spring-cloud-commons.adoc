...
Commons provides a `/service-registry` actuator endpoint.
...
You can use are `client.ribbon.MaxAutoRetries`, `client.ribbon.MaxAutoRetriesNextServer`, and `client.ribbon.OkToRetryOnAllOperations` properties.
...
Commons provides a `/features` actuator endpoint.
