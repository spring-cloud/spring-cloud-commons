...
Sometimes, it might even be mandatory to apply the `@RefreshScope` annotation on some beans which can be only initialized once. For example, starting from Spring Boot 2, the default DataSource has become `com.zaxxer.hikari.HikariDataSource`, which by default is `refreshable` (defined in `RefreshScopeBeanDefinitionEnhancer`). It's because HikariDataSource can no longer be modified once the connection pool has been started. In order to refresh this kind of `immutable` beans, you will have to either annotate the beans with `@RefreshScope` or specify the list of classname(s) under the property key `spring.cloud.refresh.extra-refreshable`.
