...
If you do not have Caffeine in the classpath, but have https://github.com/stoyanr[`com.stoyanr:evictor`] (which comes automatically with `spring-cloud-starter-loadbalancer`), the default, Evictor-based, implementation will be used.
...
If you are using `spring-cloud-starter-loadbalancer`, the cache will be set up and enabled for you by default, but if you are using just the loadbalancer module dependency (and not the starter), that will not be done.
