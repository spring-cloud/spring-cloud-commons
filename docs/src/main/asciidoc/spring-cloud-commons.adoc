...
Sometimes, it might even be mandatory to apply `@RefreshScope` annotation on some beans which can be only initialized once. For example, the default Spring Boot DataSource is now `com.zaxxer.hikari.HikariDataSource`, which by default is `refreshable` (defined in `RefreshScopeBeanDefinitionEnhancer`), as HikariDataSource can no longer be modified once it's started. In order to refresh this kind of `immutable` beans, you will have to either annotate the beans with `@RefreshScope` or specify the list of class names under the property key `spring.cloud.refresh.extra-refreshable`.
