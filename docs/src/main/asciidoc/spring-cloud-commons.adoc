...
To enable weighted load-balancing, we provide the `WeightedServiceInstanceListSupplier`. We use `WeightFunction` to calculate the weight of instance.
By default, we will try to read and parse the weight from the metadata map (the key is `weight`),
if the weight is not specified in the metadata map, then we will default the weight of this instance to be `1`.
...
You could use this sample configuration to make all instances have a random weight:
