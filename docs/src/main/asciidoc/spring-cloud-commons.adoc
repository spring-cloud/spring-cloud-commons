...
=== LoadBalancer Callbacks
You can implement the `LoadBalancedCallExecution.Callback` and add it as a `@Bean` to your project configuration to defined actions that should be executed after a load-balanced call has completed.
The `onComplete()` method that you can implement takes a `LoadBalancedCallExecution` as an argument that
contains all available data on the completed load-balanced call, including the LoadBalancer `Request`, LoadBalancer `Response` and `CompletionContext`.
