...
=== Spring WebFlux WebClient as a Reactive Load Balancer Client

`WebClient` can be configured to use the `ReactiveLoadBalancer`. `ReactiveLoadBalancerExchangeFilterFunction` and `WebClient.Builder` with `@LoadBalanced` qualifier are auto-configured if `spring-cloud-loadbalancer` is on the classpath. If you do not provide bean of type `ServiceInstanceSupplier`, DiscoveryClientServiceInstanceSupplier bean will be autoconfigured which uses Spring  Discovery Client abstraction to find available instances. The following example shows how to configure a Round-robin Reactive Load Balancer client for two services, `service-one` and `service-two`:

[source,java,indent=0]
----
@LoadBalancerClients(value = { @LoadBalancerClient("service-one"),
		@LoadBalancerClient("service-two") }, defaultConfiguration = DefaultLoadBalancerConfig.class)
class LoadBalancerClientConfiguration {
}

class DefaultLoadBalancerConfiguration {
	@Bean
...
class MyClass {
    ...
    public Mono<String> doOtherStuff() {
        return WebClient.builder().baseUrl("http://service-one")
            .filter(lbFunction)
            .build()
            .get()
            .uri("/get")
