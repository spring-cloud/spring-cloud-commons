...
IMPORTANT: If you want to use a `@LoadBalanced WebClient.Builder` it is recommended that you add the `org.springframework.cloud:spring-cloud-loadbalancer` dependency to your project.
Then, a reactive `ReactorLoadBalancerClient` will be used underneath.
Otherwise, the request will be handled by a non-reactive `@LoadBalancerClient`.
...
from discovery service without caching them. We recommend enabling caching in your project
if you use `ReactorLoadBalancerClient`.
...
==== Spring WebFlux WebClient with Reactive Load Balancer Client

`WebClient` can be configured to use the `ReactorLoadBalancerClient`.
...
If you you don't have `org.springframework.cloud:spring-cloud-loadbalancer` in your project, you can still use `WebClient` with the `LoadBalancerClient`. `LoadBalancerExchangeFilterFunction`
will be auto-configured if `spring-webflux` is on the classpath.
