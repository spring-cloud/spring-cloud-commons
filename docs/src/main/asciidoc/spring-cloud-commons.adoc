...
===== Retry-aware instance selection
...
You can also instantiate `RetryAwareServiceInstanceListSupplier` on your own. Then, apart from the constructor that allows for passing only one delegate, you can also use `RetryAwareServiceInstanceListSupplier(ServiceInstanceListSupplier delegate, ServiceInstanceListSupplier secondaryDelegate)`. If you do, the secondary delegate will be used for providing instances during retries.

You can use the builder we provide to easily instantiate suppliers, either with a secondary delegate:
...
WARNING: If you opt for instantiating `RetryAwareServiceInstanceListSupplier` on your own, make sure you set the value of `spring.cloud.loadbalancer.retry.avoid-previous-instance` to `false` so that we do not instantiate one for you automatically.
