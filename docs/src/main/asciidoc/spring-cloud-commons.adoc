...
mechanism, `ReactiveLoadBalancer` interface has been added and a *Round-Robin-based*, *Weighted* and *Random* implementations
...
In addition, you can also choose to switch to WeightedRandomLoadBalancer:

[[weighted-loadbalancer-configuration]]
[source,java,indent=0]
----
public class CustomLoadBalancerConfiguration {
...
By default, `WeightedLoadBalancer` parses instance weight from metadata (for example, `eureka.instance.metadata-map.weight`).
You can specify the weight calculation strategy by setting the `weightFunction` (implementing `ToIntFunction<ServiceInstance>` interface).

For example, you can set `weightFunction` to implement slow-start load balancing that gradually increases the weight over time.
[[slowstart-weightFunction-configuration]]
[source,java,indent=0]
----
public class CustomLoadBalancerConfiguration {

    static final long MAX_WEIGHT = 100;
    ...
    ReactorLoadBalancer<ServiceInstance> weightedLoadBalancer(Environment environment,
        ...
        return new WeightedLoadBalancer(
                ...
                name,
                weightFunction(System.currentTimeMillis()));
    }

    ToIntFunction<ServiceInstance> slowStartWeightFunction(long startTime) {
        return serviceInstance -> (int) Math.min(System.currentTimeMillis() - startTime, MAX_WEIGHT);
    }
}
----
