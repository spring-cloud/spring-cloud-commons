...
=== Spring Cloud Mismatch Verification

Due to the fact that some users have problem with setting up Spring Cloud application, we've decided
to add a mismatch verification mechanism. It will break if your current setup is not compatible
...
- Spring Boot in an incompatible version [1.5.16.RELEASE] for this release train was found
...
- Change Spring Boot version to one of the following versions [2.0,2.1], where e.g. [2.0] means Spring Boot in version [2.0.x] and [x] is the latest patch version of Spring Boot (e.g. 2.0.6.RELEASE).
