...
public class LoadBalancerRequestFactoryTests {
	...
	public void testNullTransformers() throws Exception {
		...
		Assert.assertEquals("request is different even though there were no transformers", request,
	...
	public void testEmptyTransformers() throws Exception {
		...
		verify(execution).execute(httpRequestCaptor.capture(), eq(body));
		assertEquals("request is different even though there were no transformers", request,
				httpRequestCaptor.getValue());
	}
	...
	public void testOneTransformer() throws Exception {
		...
		when(transformer1.transformRequest(request, instance, loadBalancer)).thenReturn(transformedRequest1);
	...
	public void testTwoTransformers() throws Exception {
		...
		when(transformer1.transformRequest(request, instance, loadBalancer)).thenReturn(transformedRequest1);
		when(transformer2.transformRequest(transformedRequest1, instance, loadBalancer))
