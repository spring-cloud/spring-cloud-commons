...
public class RetryLoadBalancerInterceptorTest {
	...
	public void interceptDisableRetry() throws Throwable {
		...
		when(this.client
			.execute(eq("foo"), eq(serviceInstance), any(LoadBalancerRequest.class)))
			.thenThrow(new IOException());
		this.lbProperties.setEnabled(false);
		RetryLoadBalancerInterceptor interceptor = new RetryLoadBalancerInterceptor(this.client, this.lbProperties,
			this.lbRequestFactory, this.loadBalancedRetryFactory);
		byte[] body = new byte[] {};
	...
	public void interceptInvalidHost() throws Throwable {
		...
		RetryLoadBalancerInterceptor interceptor = new RetryLoadBalancerInterceptor(this.client, this.lbProperties, this.lbRequestFactory,
	...
	public void interceptSuccess() throws Throwable {
		...
		this.lbProperties.setEnabled(true);
		RetryLoadBalancerInterceptor interceptor = new RetryLoadBalancerInterceptor(this.client, this.lbProperties,
			this.lbRequestFactory, new MyLoadBalancedRetryFactory(policy));
		byte[] body = new byte[] {};
		ClientHttpRequestExecution execution = mock(ClientHttpRequestExecution.class);
	...
	public void interceptRetryOnStatusCode() throws Throwable {
		...
		when(this.client.choose(eq("foo"))).thenReturn(serviceInstance);
		when(this.client
			.execute(eq("foo"), eq(serviceInstance), nullable(LoadBalancerRequest.class)))
			.
	...
	public void interceptRetryFailOnStatusCode() throws Throwable {
		...
		when(this.client.choose(eq("foo"))).thenReturn(serviceInstance);
		when(this.client
			.execute(eq("foo"), eq(serviceInstance), ArgumentMatchers.<LoadBalancerRequest<ClientHttpResponse>>any()))
			.thenReturn(clientHttpResponseNotFound);
	...
	public void retryListenerTest() throws Throwable {
		...
		when(this.client
			.execute(eq("listener"), eq(serviceInstance), any(LoadBalancerRequest.class)))
			.thenThrow(new IOException()).thenReturn(clientHttpResponse);
		this.lbProperties.setEnabled(true);
	...
	public void retryWithDefaultConstructorTest() throws Throwable {
		...
		when(this.client
			.execute(eq("default"), eq(serviceInstance), any(LoadBalancerRequest.class)))
			.thenThrow(new IOException()).thenReturn(clientHttpResponse);
		this.lbProperties.setEnabled(true);
		when(this.lbRequestFactory.createRequest(any(), any(), any()))
			.thenReturn(mock(LoadBalancerRequest.class));
